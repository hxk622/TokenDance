...
class StateTransition:
    ...
    @classmethod
    def get_next_state(cls, current_state: AgentState, signal: Signal) -> Optional[AgentState]:
        """根据当前状态和信号获取下一个状态
...
class StateRecord:
    """状态记录 - 用于状态轨迹"""
    state: AgentState
    signal: Optional[Signal]  # 导致进入此状态的信号
    timestamp: datetime
...
class StateMachine:
    ...
    def transition(self, signal: Signal, metadata: Optional[Dict] = None) -> AgentState:
        """执行状态转移
