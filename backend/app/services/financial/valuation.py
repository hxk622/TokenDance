...
class HistoricalValuation:
    ...
    pe_percentile: Optional[float] = None  # 当前 PE 在历史中的百分位
    
    # PB 历史区间
    pb_min_5y: Optional[float] = None
    pb_max_5y: Optional[float] = None
    pb_avg_5y: Optional[float] = None
    pb_percentile: Optional[float] = None
    
    # PS 历史区间
    ps_min_5y: Optional[float] = None
    ps_max_5y: Optional[float] = None
    ps_avg_5y: Optional[float] = None
...
class IndustryComparison:
    ...
    industry_ps_avg: Optional[float] = None
    
    # 相对行业的溢价/折价
    pe_premium: Optional[float] = None  # 正数表示溢价，负数表示折价
    pb_premium: Optional[float] = None
    ps_premium: Optional[float] = None
    
    # 行业排名
    pe_rank: Optional[int] = None
    pe_rank_total: Optional[int] = None
...
class DCFValuation:
    ...
    # 计算结果
    intrinsic_value: Optional[float] = None  # 每股内在价值
...
class ValuationAnalyzer:
    ...
    def _estimate_target_price(self, result: ValuationResult) -> Optional[float]:
        """估算目标价"""
        estimates = []
...
# 单例实例
_valuation_analyzer_instance: Optional[ValuationAnalyzer] = None
