...
class RankingResult:
    ...
    # 目标公司排名 (如果指定)
    target_rank: RankingItem | None = None
...
class IndustryRankingService:
    ...
    async def _get_metric_value(self, symbol: str, metric: RankingMetric) -> float | None:
        """获取指标值"""
        import random
    ...
    async def _calculate_composite_score(
        ...
        # Mock 各项得分 (0-100)
        score = 0
        for _metric, weight in weights.items():
...
# 全局单例
_industry_ranking_service: IndustryRankingService | None = None
