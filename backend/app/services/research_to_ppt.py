...
class ResearchToPPTConverter:
    ...
    def _create_toc_slide(self, findings: ResearchFindings) -> SlideContent:
        ...
        for i, finding in enumerate(findings.key_findings[:7], 1):  # 最多7项
    ...
    def _create_content_slides(
        ...
        for i, finding in enumerate(findings.key_findings):
            ...
            slide = SlideContent(
                type=SlideType.CONTENT,
                title=finding.title,
                subtitle=f"核心发现" if is_important else None,
    ...
    def _create_data_slides(
        ...
        if number_data and len(number_data) >= 3:
            ...
            # 使用表格展示
            points = [f"{l}: {v}{u or ''}" for l, v, u in zip(labels, values, units)]
    ...
    def _create_thank_you_slide(self, author: Optional[str]) -> SlideContent:
        """创建感谢页"""
        subtitle = None
        if author:
