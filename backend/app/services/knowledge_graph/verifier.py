...
from .models import (
    ...
    ResearchKnowledgeGraph, ClaimVerification, ConfidenceLevel
)
...
class CrossSourceVerifier:
    ...
    def __init__(
        self,
        graph: Optional[ResearchKnowledgeGraph] = None,
...
只返回一个词，不要解释。"""
    ...
    def _make_verdict(
        ...
        # 收集唯一来源
        support_sources = set(s[0] for s in supporting if s[0])
        contradict_sources = set(s[0] for s in contradicting if s[0])
