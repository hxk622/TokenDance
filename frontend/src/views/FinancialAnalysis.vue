...
// Key points - computed from backend sentiment analysis
const keyPoints = computed(() => {
  ...
  // Use backend-provided key points
  const bullishPoints = (sentiment.analysis.key_bullish_points || []).map((point, index) => ({
    content: point,
    summary: point.length > 50 ? point.slice(0, 50) + '...' : point,
    // Find supporting posts by matching bullish sentiment
    supportingPosts: sentiment.posts
      .filter(p => p.sentiment_label === 'bullish')
      .slice(index * 2, index * 2 + 2)
      .map(p => p.id)
      .filter((id): id is string => !!id),
  }))

  const bearishPoints = (sentiment.analysis.key_bearish_points || []).map((point, index) => ({
    content: point,
    summary: point.length > 50 ? point.slice(0, 50) + '...' : point,
    supportingPosts: sentiment.posts
      .filter(p => p.sentiment_label === 'bearish')
      .slice(index * 2, index * 2 + 2)
      .map(p => p.id)
      .filter((id): id is string => !!id),
  }))

  return { bullish: bullishPoints, bearish: bearishPoints }
})
